...
def get_purchase_charts(purchases):
    ...
    ret[0]['chart'][0]['color'][i] = ['' for _ in range(len(purchases))]
    for i in range(len(purchases)):
        ret[0]['chart'][0]['color'][i] = 'red' if purchases[i][0].purchase_category else 'blue'
    ret[0]['chart'][0]['data'] = list(map(lambda a: a[0].prices, purchases))
    ...
    ret[1]['chart'][0]['color'][i] = ['' for _ in range(len(purchases))]
    for i in range(len(purchases)):
        ret[1]['chart'][0]['color'][i] = 'red' if purchases[i][0].purchase_category else 'blue'
    ret[1]['chart'][0]['data'] = list(map(
        lambda a: sum(map(lambda contract: contract.price, a[1]['contracts']))
        if a[1]['is_winner'] else -sum(map(lambda contract: contract.price, a[1]['contracts'])), purchases))
