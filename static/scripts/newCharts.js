class ChartJSEnhancements {
    ...
    resetZoom(t = !0, s = {}) {
        this.chartjs_object.options.scales.xAxes[0].ticks.min = void 0, this.chartjs_object.options.scales.xAxes[0].ticks.max = void 0;
        for (let t in this.chartjs_object.options.scales.yAxes) this.chartjs_object.options.scales.yAxes[t].ticks.min = void 0, this.chartjs_object.options.scales.yAxes[t].ticks.max = void 0;
        t && this.chartjs_object.update(Object.assign({duration: this.quick_mode ? 0 : 150}, s)), this.setMinMax()
    }

    unselectPoints(t = !0) {
        this.selected_points = [], this.change_point_radius && this.datasets.forEach(t => {
            t.pointRadius = Array(t.data.length), t.pointBorderWidth = Array(t.data.length)
        }), t && this.chartjs_object.update({duration: this.quick_mode ? 0 : 300}), void 0 !== this.after_unselect_handler && this.after_unselect_handler()
    }
    ...
    clickHandler(t) {
        if ("mousedown" === t.type) {
            ...
            if (this.chartjs_object.unbindEvents(), this.rect_selector.startX = t.offsetX, this.rect_selector.startY = t.offsetY, this.drag = !0, this.initial_time = t.timeStamp, "select" === this.action && (this.selecting_points = !0), "pan" === this.action && (this.panning = !0), this.resetPoints(), this.mouse_button_value = t.buttons, this.panning) {
                if (this.data_length > 8e3) for (let t = 0; t < this.points_backup.length; ++t) this.points_backup[t] = this.datasets[t].data, this.datasets[t].data = this.datasets[t].data.filter((t, s) => 0 == (7 & s));
                this.setMinMax()
        ...
        if ("mouseup" === t.type && (this.ignore_mouse_button || this.mouse_button_value === this.action_buttons[this.action])) {
            ...
            if (!this.panning && s >= this.time_limit) {
                ...
                let t = {min: Math.min(...this.selector_points.x), max: Math.max(...this.selector_points.x)}, s = {};
                ...
                if (this.selecting_points) {
                    if ("select" === this.action) {
                        let e = Array(this.data_length), i = 0;
            ...
            } else if (s < this.time_limit) this.unselectPoints(); else if (this.panning && this.data_length > 8e3) for (let t = 0; t < this.points_backup.length; ++t) this.datasets[t].data = this.points_backup[t];
            this.chartjs_object.bindEvents(), this.chartjs_object.update({duration: this.quick_mode ? 0 : 150}), this.drag = !1, this.panning = !1, this.mouse_button_value = 0, this.selecting_points = !1, this.ignore_mouse_button && !this.using_shift || (this.action = this.previous_action, this.using_shift = !1)
        }
    }

    mousemoveHandler(t) {
        if (this.drag) if (this.panning) {
            if ("pan" === this.action) {
                const s = t.offsetX - this.rect_selector.startX, e = t.offsetY - this.rect_selector.startY;
    ...
    resetPoints() {
        this.selector_points.y = Object.assign({}, ...Object.keys(this.chartjs_object.scales).map(t => ({[t]: []}))), this.selector_points.x = []
    }
    ...
    initialize(t, s = "white") {
        this.temp_canvas = document.createElement("canvas"), this.temp_ctx = this.temp_canvas.getContext("2d", {alpha: !1}), this.temp_background_color = s, this.rect_selector = {}, this.selector_points = {
            x: [],
            y: {}
